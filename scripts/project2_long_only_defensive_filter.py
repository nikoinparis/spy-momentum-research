...
# Current long-only momentum settings
LOOKBACK = 60
THRESHOLD = 0.001
TARGET_VOL = 0.10
VOL_WINDOW = 40
...
def apply_defensive_filter(positions: pd.DataFrame, prices: pd.DataFrame, ma_window: int) -> pd.DataFrame:
    ...
    filtered_positions = filtered_positions.where(bullish_regime, 0.0)
...
def main():
    ...
    # Long-only + defensive filter
    filt_positions = apply_defensive_filter(lo_positions, prices, MA_WINDOW)
    filt_result = backtest_positions(
        asset_log_returns=log_returns,
        positions=filt_positions,
        transaction_cost_bps=TCOST_BPS,
    )
    filt_returns = apply_vol_targeting(filt_result.strategy_log_returns, TARGET_VOL, VOL_WINDOW)
    filt_equity = np.exp(filt_returns.cumsum())
    if len(filt_equity) > 0:
        filt_equity.iloc[0] = 1.0

    summary_lo = summarize_strategy("Long-Only Momentum", lo_returns, lo_equity)
    summary_filt = summarize_strategy("Long-Only + 200d MA Filter", filt_returns, filt_equity)

    summary_df = pd.DataFrame([summary_lo, summary_filt])

    print("\n=== Step 15: Long-Only Momentum with Defensive Filter ===\n")
    print(summary_df.to_string(index=False))

    plt.figure(figsize=(12, 6))
    plt.plot(lo_equity.index, lo_equity.values, label="Long-Only Momentum")
    plt.plot(filt_equity.index, filt_equity.values, label="Long-Only + 200d MA Filter")
    plt.title("Step 15: Long-Only Momentum vs Defensive Filter")
    plt.xlabel("Date")
    plt.ylabel("Equity")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("step15_long_only_defensive_filter.png", dpi=150)
    plt.show()

    print("\nPlot saved to step15_long_only_defensive_filter.png")
